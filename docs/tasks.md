...
| URL | `url` | Required | Video, live-stream, or playlist page; Live mode requires one stream |
...
- Tasks, API requests, and the web form share one active download per normalized
  URL. The lock remains through queueing, discovery, startup, recording, Stop,
  and finalization.
